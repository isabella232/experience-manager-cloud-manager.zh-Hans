...
在从AEM应用程序内执行HTTP请求时，务必确保配置适当的超时以避免不必要的线程消耗。 很遗憾，Java的默认HTTP客户端(java.net.HttpUrlConnection)和常用的Apache HTTP组件客户端的默认行为都为从不超时，因此必须显式设置超时。 此外，作为最佳实践，这些超时不应超过60秒。
...
仅用于由AEM实现的接口（和类）使用org.osgi.annotation. *versioning.ProviderType* (或在某些情况下，类似的旧 *式注释aQute.bnd.annotation.ProviderType*)进行注释。 此规则标识通过自定义代码实现（或扩展类）此类接口的情况。
...
如Sling文档中所 [述](http://sling.apache.org/documentation/the-sling-engine/servlets.html)，不建议使用按路径绑定的Servlet。 路径绑定Servlet不能使用标准JCR访问控制，因此需要额外的安全性严格。 建议在存储库中创建节点并按资源类型注册servlet，而不是使用路径绑定servlet。
...
通常，INFO日志级别应用于划分重要操作，默认情况下，AEM配置为在INFO级别或更高级别进行日志记录。 GET和HEAD方法只应是只读操作，因此不构成重要操作。 响应GET或HEAD请求以INFO级别进行记录可能会产生明显的日志噪声，因此更难识别日志文件中的有用信息。 处理GET或HEAD请求时的日志记录应位于出错时的WARN或ERROR级别，或者位于DEBUG或TRACE级别（如果更深入的故障排除信息会有所帮助）。
...
如前所述，了解日志消息时，上下文至关重要。 使用Exception.printStackTrace()只 **导致** 堆栈跟踪输出到标准错误流，从而丢失所有上下文。 此外，在多线程应用程序（如AEM）中，如果同时使用此方法打印多个例外，则其堆栈轨迹可能会重叠，从而产生严重混淆。 例外应仅通过记录框架记录。
...
登录AEM应始终通过日志记录框架(SLF4J)完成。 直接输出到标准输出或标准错误流会丢失日志记录框架提供的结构和上下文信息，在某些情况下，可能会导致性能问题。
...
客户应将AEM内容存储库中的/libs内容树视为只读，这是一种长期的最佳实践。 修改/libs下的节点 *和属性* ，会给主要和次要更新带来重大风险。 对/ *lib的修* 改仅应由Adobe通过官方渠道进行。
...
出于安全原因，包 *含/config/和/install/* /install/的路径只能由AEM中的管理用户读取，并且只能用于OSGi配置和OSGi捆绑包。 将其他类型的内容放在包含这些区段的路径下会导致应用程序行为在管理用户和非管理用户之间无意中发生变化。
...
#### OakPAL —— 默认创作模式不应为经典UI {#oakpal-default-authoring}
...
OSGi配置定 `com.day.cq.wcm.core.impl.AuthoringUIModeServiceImpl` 义AEM中的默认创作模式。 自AEM 6.4起，经典UI已弃用，现在将默认创作模式配置为经典UI时，会引发问题。

#### OakPal —— 具有对话框的组件应具有触屏UI对话框 {#oakpal-components-dialogs}
...
AEM现代化工具文档提供了如何将组件从经典UI转换为触屏UI的文档和工具。 有关更 [多详细信息，请参](https://opensource.adobe.com/aem-modernize-tools/pages/tools.html) 阅AEM现代化工具。

#### OakPal —— 软件包不应混合可变和不可变内容 {#oakpal-packages-immutable}
...
### OakPal —— 不应使用反向复制代理 {#oakpal-reverse-replication}
...
### SonarQube —— 不应使用Sling调度程序 {#sonarqube-sling-scheduler}
...
### SonarQube - SonarQube —— 不应使用AEM已弃用的API {#sonarqube-aem-deprecated}
...
AEM API表面处于不断修订的状态，用于标识不建议使用的API，因此被视为已弃用。

在很多情况下，这些API使用标准Java@ *Deprecated* annotation弃用，如由标识 `squid:CallToDeprecatedMethod`。

但是，在某些情况下，AEM上下文中已弃用API，但其他上下文中可能不弃用。 此规则标识此第二类。
